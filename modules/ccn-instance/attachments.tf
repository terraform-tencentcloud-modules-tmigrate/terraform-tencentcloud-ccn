...
#  for_each = var.vpn_gateway_ccn_routes
#  destination_cidr_block = try(each.value.cidr, "192.168.0.0/24")
#  route_id               = try(each.value.route_id, "vpnr-xxxx")
#  status                 = try(each.value.route_status, "ENABLE")
